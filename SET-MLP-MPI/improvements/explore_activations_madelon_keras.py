...
def read_data():
    ...
    val_data_url = '../../data/Madelon/madelon_valid.data'
    train_resp_url = '../../data/Madelon/madelon_train.labels'
    val_resp_url = '../../data/Madelon/madelon_valid.labels'
    test_data_url = '../../data/Madelon/madelon_test.data'

    # This data isn't standard csv so using numpy to load it as text
    # Download the data straight from the web
    x_train = np.loadtxt(train_data_url)
    y_train = np.loadtxt(train_resp_url)
    x_val = np.loadtxt(val_data_url)
    y_val = np.loadtxt(val_resp_url)
    x_test = np.loadtxt(test_data_url)
...
[noPar1, wm1] = createWeightsMask(20,500, 100)
[noPar2, wm2] = createWeightsMask(20,100, 100)
[noPar3, wm3] = createWeightsMask(20,100, 100)

model = Sequential()
model.add(Dense(400, name="sparse_1",kernel_constraint=MaskWeights(wm1),weights=w1, input_shape=(500,)))
model.add(SReLU(name="srelu1",weights=wSRelu1))
model.add(Dropout(0.3))
model.add(Dense(100, name="sparse_2",kernel_constraint=MaskWeights(wm2),weights=w2))
model.add(SReLU(name="srelu2",weights=wSRelu2))
model.add(Dropout(0.3))
model.add(Dense(400, name="sparse_3",kernel_constraint=MaskWeights(wm3),weights=w3))
model.add(SReLU(name="srelu3",weights=wSRelu3))
model.add(Dropout(0.3))
model.add(Dense(1, name="dense_4", weights=w4)) #please note that there is no need for a sparse output layer as the number of classes is much smaller than the number of input hidden neurons
model.add(Activation('sigmoid'))

model.load_weights('madelon_weights_fulltraining.h5')
...
incorrects = np.nonzero(y_pred.reshape((-1,)) != Y_test)[0]
incorrect_classes = y_pred[incorrects]
correct_classes = Y_test[incorrects]
for i in range(0, 2):
...
np.savetxt("Weights1_madelon.txt", weights[1])
weights[2] = model.get_layer("sparse_2").get_weights()[0]
np.savetxt("Weights2_madelon.txt", weights[2])
weights[3] = model.get_layer("sparse_3").get_weights()[0]
np.savetxt("Weights3_madelon.txt", weights[3])
weights[4] = model.get_layer("dense_4").get_weights()[0]
np.savetxt("Weights4_madelon.txt", weights[4])
b ={}
b[1] = model.get_layer("sparse_1").get_weights()[1]
np.savetxt("Biases1_madelon.txt", b[1])
b[2] = model.get_layer("sparse_2").get_weights()[1]
np.savetxt("Biases2_madelon.txt", b[2])
b[3] = model.get_layer("sparse_3").get_weights()[1]
np.savetxt("Biases3_madelon.txt", b[3])
b[4] = model.get_layer("dense_4").get_weights()[1]
np.savetxt("Biases4_madelon.txt", b[4])
srelu_weights = {}
srelu_weights[1] = np.loadtxt("SReluWeights1_madelon.txt")
srelu_weights[2] = np.loadtxt("SReluWeights2_madelon.txt")
srelu_weights[3] = np.loadtxt("SReluWeights3_madelon.txt")
...
for k, w in weights.items():
    if k==2:
        get_nth_layer_output = K.function([model.layers[0].input], [model.layers[3].output])
    ...
    if k==1:
        get_nth_layer_output = K.function([model.layers[0].input], [model.layers[0].output])
        inputs = get_nth_layer_output([X_test])[0]
        mean_input = inputs.mean(axis=0)
        ...
        mean_activation = activations.mean(axis=0)
    if k==3:
        get_nth_layer_output = K.function([model.layers[0].input], [model.layers[6].output])
        inputs = get_nth_layer_output([X_test])[0]
        mean_input = inputs.mean(axis=0)
        ...
        mean_activation = activations.mean(axis=0)
    if k==4:
        get_nth_layer_output = K.function([model.layers[0].input], [model.layers[9].output])
        inputs = get_nth_layer_output([X_test])[0]
        mean_input = inputs.mean(axis=0)
    ...
    if k != 4:
        ...
        t2 = np.percentile(edges, 70)
        idxs = (-edges).argsort()[:100]
        print(
            f"Removing {edges[(edges<t2)].shape[0]} neurons and {incoming_edges[(edges<t2)].sum()} weights , weighted sum threshold is {t}, connection threshold is {t_connections}")
        edges = np.where((edges<t2) , 0, edges)
...
model.save_weights('madelon_weights_fulltraining_pruned_2.h5')
