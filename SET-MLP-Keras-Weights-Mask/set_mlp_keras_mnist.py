...
class SET_MLP_MNIST:
    ...
    def train(self):
        ...
        for epoch in range(0, self.maxepoches):
            ...
            K.clear_session()
            self.create_model()
            np.savetxt("SReluWeights1_mnist.txt", self.wSRelu1)
            np.savetxt("SReluWeights2_mnist.txt", self.wSRelu2)
            np.savetxt("SReluWeights3_mnist.txt", self.wSRelu3)
        self.model.save_weights('mnist_weights_fulltraining.h5')
...
if __name__ == '__main__':
    ...
    np.savetxt("results/set_new_version_mlp_srelu_sgd_mnist_acc.txt", np.asarray(model.accuracies_per_epoch))
