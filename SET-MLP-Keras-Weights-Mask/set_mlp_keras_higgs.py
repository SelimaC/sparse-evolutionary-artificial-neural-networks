...
class SET_MLP_HIGGS:
    def __init__(self):
        ...
        [self.noPar1, self.wm1] = createWeightsMask(self.epsilon, 28, 1000)
    ...
    def train(self):
        ...
        for epoch in range(0, self.maxepoches):
            ...
            K.clear_session()
            self.create_model()
            np.savetxt("SReluWeights1_higgs.txt", self.wSRelu1)
            np.savetxt("SReluWeights2_higgs.txt", self.wSRelu2)
            np.savetxt("SReluWeights3_higgs.txt", self.wSRelu3)
        self.model.save_weights('higgs_weights_fulltraining.h5')

        self.accuracies_per_epoch=np.asarray(self.accuracies_per_epoch)

    def read_data(self, n_training_samples=105000, n_testing_samples=50000):
