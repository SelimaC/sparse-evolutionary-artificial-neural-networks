...
class SET_MLP:
    ...
    def _back_prop(self, z, a, masks, y_true):
        ...
        total_backprop +=(t6 - t5).total_seconds()
        update_params = {
    ...
    def fit(self, x, y_true, x_test, y_test, testing=True, save_filename=""):
        ...
        for i in range(self.epochs):
            ...
            back_propagation_numpy_total_time = 0
            back_propagation_total_time = 0
            feed_forward_total_time = 0
            ...
            for j in range(x.shape[0] // self.batch_size):
                ...
                feed_forward_total_time += (t4 - t3).total_seconds()

                t3 = datetime.datetime.now()
                bp_time = self._back_prop(z, a, masks,  y_[k:l])
                t4 = datetime.datetime.now()
                back_propagation_total_time += (t4 - t3).total_seconds()

                back_propagation_numpy_total_time += bp_time

            t2 = datetime.datetime.now()

            print("\nSET-MLP Epoch ", i)
            print("Training time: ", t2 - t1)
            print("Feedforward total time: ", feed_forward_total_time)
            print("Backpropagation numpy time: ", back_propagation_numpy_total_time)
            print("Backpropagation total time: ", back_propagation_total_time)
    ...
    def fit_generator(self, x, y_true, x_test, y_test, testing=True, save_filename=""):
        ...
        for i in range(self.epochs):
            ...
            for j in range(x.shape[0] // self.batch_size):
                ...
                bp_time = self._back_prop(z, a, masks, y_b)
                back_propagation_total_time += bp_time
            t2 = datetime.datetime.now()
            print("\nSET-MLP Epoch ", i)
            print("Training time: ", t2 - t1)
